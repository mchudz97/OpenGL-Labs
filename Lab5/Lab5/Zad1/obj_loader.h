	...
	) 
{
	...
	std::vector<glm::vec2> temp_uvs;
	std::vector<glm::vec3> temp_normals;
	std::vector<glm::vec3> temp_vert_idx;
	std::vector<glm::vec2> temp_uvs_idx;
	std::vector<glm::vec3> temp_normals_idx;
	...
	if (loadOBJ(path, temp_vertices, temp_uvs, temp_normals)) 
	{
		for (int i = 0; i < temp_vertices.size(); i++) {
			...
			for (int j = 0; j < temp_vert_idx.size(); j++) {
				if (temp_vertices[i] == temp_vert_idx[j]) {
					idx.push_back(j + 1);
					isIncluded = true;
				}
			...
			if (!isIncluded) {
				temp_vert_idx.push_back(temp_vertices[i]);
				temp_uvs_idx.push_back(temp_uvs[i]);
				temp_normals_idx.push_back(temp_normals[i]);
				idx.push_back(actualIdx++);
			}
		}
		return true;
	}
