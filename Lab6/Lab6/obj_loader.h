...
void calculateTangents(std::vector<Vertex>& outVertices);
	...
	if (loadOBJ(path, temp_vertices, temp_uvs, temp_normals))
	{
		...
		if (setTangents) 
		{
			...
			for (int i = 0; i < temp_vertices.size(); i += 3) 
			{
				...
				glm::vec3 edge1 = temp_vertices[i + 1] - temp_vertices[i];
				glm::vec3 edge2 = temp_vertices[i + 2] - temp_vertices[i];
				glm::vec2 deltaUV1 = temp_uvs[i + 1] - temp_uvs[i];
				glm::vec2 deltaUV2 = temp_uvs[i + 2] - temp_uvs[i];
			...
			for (int i = 0; i < temp_vertices.size(); i++) {
				...
				if (!isIncluded) {
					...
					v.v_tangent = tangents[i];
					v.v_bitangent = bitangents[i];
