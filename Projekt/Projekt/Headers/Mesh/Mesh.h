...
public:
	...
	void renderStacked(Shader& shader, glm::mat4 transform) {
		BaseMesh::render(shader);
		shader.setMat4(transform, "modelTransform");
		glBindVertexArray(this->idVAO);
	...
	void shadowRenderStacked(Shader& shader, glm::mat4 transform) 
	{
		shader.setMat4(transform, "modelTransform");
		shader.setBool(false, "isInstanced");
		glBindVertexArray(this->idVAO);
		if (indicesCount)
			glDrawElements(GL_TRIANGLES, this->indicesCount, GL_UNSIGNED_INT, 0);
		else
...
void exampleTranslationFunc(glm::mat4 &t, float step) {
	...
	t = glm::translate(glm::mat4(1.f), glm::vec3(sin(step) * range, 2, cos(step) * range));
