...
public:
	...
	virtual void shadowRender(Shader& shader) {}
	virtual void renderToMap(Shader& shader) 
	{
		if (this->defaultTexture)
		{
			glActiveTexture(GL_TEXTURE0);
	...
	std::vector<glm::vec3> getVerts()
	{
		...
		for (unsigned i : this->indices)
			v.push_back(this->verts[i].v_position);
		return v;
	}

	glm::vec2 getMaxRanges() 
	{
		GLfloat r = 0;
		GLfloat h = 0;
		for (unsigned i : this->indices) 
		{
			glm::vec3 p = this->verts[i].v_position;
			GLfloat new_r = sqrt(p.x * p.x + p.z * p.z);
			if (new_r > r)
				r = new_r;
			if (p.y > h)
				h = p.y;
